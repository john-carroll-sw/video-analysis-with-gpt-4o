...
def execute_video_processing(st, segment_path, system_prompt, user_prompt, temperature, frames_per_second, 
    ...
    with st.spinner(f"Analyzing video segment {segment_num + 1}"):
        ...
        with st.spinner("Extracting frames..."):
            ...
            if save_frames:
                # Get the video's analysis directory (e.g., video/samsungpowersupply_analysis)
                video_analysis_dir = os.path.dirname(analysis_dir)  # Go up one level from analysis_subdir
                frames_dir = os.path.join(video_analysis_dir, "frames")  # Create frames directory at same level as segments/analysis
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### Analysis Range (Optional)")
...
if st.session_state.show_chat and st.session_state.analyses is not None:
    ...
    analysis_tab, chat_tab = st.tabs(["Video Analysis Results", "Chat"])
